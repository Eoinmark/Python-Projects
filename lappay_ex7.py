...
This code is an upgrade to the emulated bookstore inventory system code used in the last exercise which implemented the use of a dictionary. Just like last time, users can input a book code which serves as the key in the book inventory dictionary, in which corresponds to a book title, author and quantity contained as a list for the key value. The program furthermore use defined functions for each choice of action the user do from the available choices in  the menu. This time, persistence is provided in the program by allowing the inventory system to be saved in a dat file. The creating, writing and saving of the inventory to the inventory.dat file is performed using an imported module.
"""
#Import module
import lappay_ex7_files
...
def restockBook():
    ...
    if empty == True:                      #checks whether the inventory is empty through the empty flag
        ...
        if flag == False:                  # if the book is not in the inventory.
            ...
            book_inventory[book_code][3] = str(new_amount)                   #stores the restocked amount in the list value stored in the book inventory dictionary
    ...
    if empty == True:                      # if the inventory is empty, tells the user there is nothing to consume
        ...
        if flag == False:                     #if the book code is not stored in the dictionary, tells the user that the book does not exist
            ...
            if new_amount < 0:                                      #if the amount consumed is greater than the current stock (that is if their difference is negative or less than 0), then informs the user that there is not enough stock
                ...
                book_inventory[book_code][3] = str(new_amount)
                print("New stock of ", book_inventory[book_code][1] ," : ", book_inventory[book_code][3]) 
...
#Using conditionals to do specific functions for each choice of the user    
    if choice == 0:       #break the loop if the user chooses to exit
        ...
        lappay_ex7_files.loadInventory(book_inventory) #the loadinventory function from the imported module is used
        empty = False
    elif choice == 8:
        lappay_ex7_files.saveInventory(book_inventory) #the saveInventory function from the imported module is used
    else:
